...
class ManageScheduleOneDoctor extends Component {
  ...
  handleSaveSchedule = async () => {
    ...
    if (res && res.errCode === 0) {
      ...
      // Trường hợp có lịch trùng
      if(this.props.language=="en"){
        toast.error("There are conflicting appointments at the selected times!");
      }else{
        toast.error("Đã có lịch hẹn trùng với thời gian bạn chọn!");
      }

      // Hiển thị chi tiết các lịch trùng
      if (res.conflictBookings && res.conflictBookings.length > 0) {
        let conflictTimes = res.conflictBookings.map(item => item.timeText).join(", ");

        if(this.props.language=="en"){
          toast.warning(`Conflicting times: ${conflictTimes}`);
        }else{
          toast.warning(`Thời gian trùng lịch: ${conflictTimes}`);
        }
      }
    } else {
      if(this.props.language=="en"){
