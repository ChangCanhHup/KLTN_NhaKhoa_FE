...
class DoctorSchedule extends Component {
  ...
  async componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (this.props.doctorIdFromParent !== prevProps.doctorIdFromParent) {
      ...
      if(res && res.data){
          let copyRes=[...res.data];
          // Lọc các khung giờ sau giờ hiện tại và khả dụng
          copy_allAvailableTime = copyRes.filter((element) =>
            element.value > currentHour && element.isAvailable
          );
          this.setState({
            allAvailableTime: copy_allAvailableTime ? copy_allAvailableTime : [],
          });
                ...
                <>
                  <div className="time-content-btns">
                    {allAvailableTime.map((item, index) => {
                      ...
                      if (!item.isAvailable) {
                        // Nếu không khả dụng, kiểm tra lý do
                        if (item.isBusy) {
                          // Bác sĩ bận với công việc khác
                          buttonClass += " busy";
                          tooltipText = language === LANGUAGES.VI
                            ? "Bác sĩ bận"
                            : "Doctor is busy";
                        ...
                        return (
                          <div key={index} className="time-button-container" title={tooltipText}>
                            <button
