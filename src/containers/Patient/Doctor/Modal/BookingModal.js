...
class BookingModal extends Component {
  ...
  handleConfirmBooking = async () => {
    ...
    else if(res && res.errCode === 3)
    {
      this.setState({ isShowLoading: false });
        if(language==="en"){
          toast.warn("This time slot is already booked. Please choose another time!");
        }else{
            toast.warn("Thời gian này đã có lịch hẹn, vui lòng chọn khung thời gian khác!");
        }
    }
    else if(res && res.errCode === 4)
    {
      this.setState({ isShowLoading: false });
        if(language==="en"){
          toast.warn("The doctor is busy with other work at this time. Please choose another time!");
        }else{
            toast.warn("Bác sĩ đã có lịch làm việc khác vào thời gian này, vui lòng chọn khung thời gian khác!");
        }
    }
    else {
      this.setState({ isShowLoading: false });
      ...
      if (res && res.errMessage) {
        if(language==="en"){
          toast.error(`Booking error: ${res.errMessage}`);
        }else{
          toast.error(`Lỗi đặt lịch: ${res.errMessage}`);
        }
      } else {
        if(language==="en"){
          toast.error("Book a new appointment error! Please try again.");
        }else{
